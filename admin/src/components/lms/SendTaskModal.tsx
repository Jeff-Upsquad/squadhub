...
import type { LmsItemShare, Role, User } from '@squadhub/shared';

type Lesson = { id: string; title: string; blocks?: any[] };
interface Props {
  itemId: string;
  itemTitle: string;
  itemKind: string;   // 'post' | 'course'
  itemTrack: string;  // 'learning' | 'sop'
  lessons: Lesson[];
  onClose: () => void;
}
...
// --- section heading parse (mirrors LearningItemView's reader-rail logic) -----
// The reader scans the rendered lesson for h1–h3 in document order and anchors
// each as `sec-{i}-{slug}`. We derive the same anchors from the stored Tiptap
// JSON so a section task deep-links to the right heading.
...
export default function SendTaskModal({ itemId, itemTitle, itemKind, itemTrack, lessons, onClose }: Props) {
  ...
  // Default task title from the chosen scope, unless the admin edited it.
  ...
  const send = useMutation({
    mutationFn: () =>
      api.post(`/admin/lms/items/${itemId}/task-sends`, {
        scope,
    ...
    onSuccess: (res: any) => {
      qc.invalidateQueries({ queryKey: ['lms-task-sends', itemId] });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => !send.isPending && onClose()}>
      <div className="flex max-h-[88vh] w-full max-w-lg flex-col rounded-xl border border-divider bg-surface shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="border-b border-divider px-5 py-4">
          <h2 className="text-lg font-bold text-foreground">Send as task</h2>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
          {/* Scope */}
          <div>
            <label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-foreground-dim">What to send</label>
              ...
              {(['item', 'lesson', 'section'] as Scope[]).map((s) => (
                <button
                  ...
                  className={`rounded-md border px-3 py-1.5 text-[12.5px] font-medium capitalize transition ${
                    scope === s ? 'border-ink bg-ink text-white' : 'border-divider bg-surface text-foreground-muted hover:bg-surface-alt'
                  }`}
                >
          ...
          {(scope === 'lesson' || scope === 'section') && (
            <div>
              <label className="mb-1 block text-[11px] font-medium uppercase tracking-wider text-foreground-dim">
          ...
          {scope === 'section' && (
            <div>
              <label className="mb-1 block text-[11px] font-medium uppercase tracking-wider text-foreground-dim">Section (heading)</label>
              {headings.length === 0 ? (
          ...
          {/* Title */}
          <div>
            <label className="mb-1 block text-[11px] font-medium uppercase tracking-wider text-foreground-dim">Task title</label>
            <input
          ...
          <div className="flex flex-wrap items-end gap-4">
            <div>
              <label className="mb-1 block text-[11px] font-medium uppercase tracking-wider text-foreground-dim">Due date (optional)</label>
              <input
                type="date"
                value={dueDate}
                onChange={(e) => setDueDate(e.target.value)}
                className="rounded-md border border-divider bg-surface px-3 py-2 text-sm focus:border-ink focus:outline-none"
              />
            </div>
            <label className="flex cursor-pointer items-center gap-2 pb-2 text-[12.5px] text-foreground">
              <input type="checkbox" checked={autoResend} onChange={(e) => setAutoResend(e.target.checked)} className="h-4 w-4 rounded border-divider-strong accent-[#0F172B]" />
          ...
          {/* Recipients */}
          <div>
            <label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-foreground-dim">Send to</label>
            ...
            {tab === 'people' ? (
              <>
                ...
                />
                {q && (
                  <ul className="mt-2 max-h-36 overflow-y-auto rounded-md border border-divider">
                    {users.filter((u) => !takenIds.has(`user:${u.id}`)).slice(0, 20).map((u) => (
                      <li key={u.id}>
                        <button type="button" onClick={() => add({ type: 'user', id: u.id, label: u.display_name || u.email, sub: u.email })} className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-surface-alt">
                          <span className="grid h-7 w-7 place-items-center rounded-full bg-canvas text-[10px] font-semibold text-foreground-muted">{(u.display_name || u.email || '?').slice(0, 2).toUpperCase()}</span>
                          <span className="flex-1 truncate"><span className="text-foreground">{u.display_name || u.email}</span><span className="ml-1.5 text-[11px] text-foreground-dim">{u.user_type}</span></span>
              ...
              <ul className="max-h-36 overflow-y-auto rounded-md border border-divider">
                {roles.filter((r) => !takenIds.has(`role:${r.id}`)).map((r) => (
                  <li key={r.id}>
                    <button type="button" onClick={() => add({ type: 'role', id: r.id, label: r.name, sub: 'Role', color: r.color })} className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-surface-alt">
            ...
            {principals.length > 0 && (
              <ul className="mt-2 space-y-1.5">
                {principals.map((p, i) => (
                  <li key={`${p.type}:${p.id}`} className="flex items-center gap-2 rounded-md border border-divider bg-surface px-2.5 py-1.5">
                    {p.type === 'role' ? (
                      ...
                      <span className="grid h-7 w-7 place-items-center rounded-full bg-canvas text-[10px] font-semibold text-foreground-muted">{p.label.slice(0, 2).toUpperCase()}</span>
                    )}
                    <span className="flex-1 truncate">
                      <span className="block truncate text-sm text-foreground">{p.label}</span>
                      {p.sub && <span className="block truncate text-[11px] text-foreground-dim">{p.sub}</span>}
                    </span>
        ...
        <div className="flex items-center justify-between border-t border-divider px-5 py-3">
          ...
          <div className="flex gap-2">
            <button onClick={onClose} disabled={send.isPending} className="rounded-lg border border-divider bg-surface px-4 py-2 text-sm text-foreground-muted hover:bg-surface-alt disabled:opacity-50">Cancel</button>
            <button onClick={() => send.mutate()} disabled={!canSend || send.isPending} className="rounded-lg bg-ink px-4 py-2 text-sm font-medium text-white hover:bg-ink-hover disabled:opacity-50">
