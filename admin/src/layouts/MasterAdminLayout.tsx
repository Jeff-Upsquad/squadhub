...
function NavLink({
  ...
  const isActive = match ? match(pathname) : end ? pathname === href : pathname.startsWith(href);
  return (
    <Link
      href={href}
...
export default function MasterAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-surface text-foreground">
      ...
      <aside className="flex w-56 flex-col border-r border-divider bg-surface">
        ...
        <nav className="flex-1 min-h-0 overflow-y-auto p-3 space-y-0.5">
          ...
          <NavLink
            ...
            match={(p) =>
              p.startsWith('/admin/subscription-cards') ||
              p === '/admin/assignments' ||
      ...
      {/* Content area */}
      <main className="flex min-h-0 flex-1 flex-col overflow-y-auto bg-canvas p-6">
        {children}
      </main>
    </div>
  );
