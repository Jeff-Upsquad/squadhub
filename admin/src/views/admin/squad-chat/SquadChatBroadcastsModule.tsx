...
export default function SquadChatBroadcastsModule() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h1 className="font-[family-name:var(--font-display)] text-2xl font-bold text-[#0F172B]">Broadcasts</h1>
      ...
      <div className="rounded-xl border border-[#E2E8F0] bg-white p-5 space-y-3">
        <div>
          <label className="block text-xs font-medium uppercase tracking-wider text-[#62748E] mb-1">Group</label>
          <select
            value={groupId}
        ...
        <div>
          <label className="block text-xs font-medium uppercase tracking-wider text-[#62748E] mb-1">Message</label>
          <textarea
            value={content}
      ...
      <div>
        <h2 className="text-xs font-medium uppercase tracking-wider text-[#62748E] mb-3">Last 30 days</h2>
        <div className="rounded-xl border border-[#E2E8F0] bg-white">
          {history.length === 0 ? (
            <div className="p-6 text-center text-sm text-[#62748E]">No recent broadcasts.</div>
          ) : (
              ...
              {history.map((m) => (
                <li key={m.id} className="px-4 py-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="text-[11px] text-[#90A1B9]">{new Date(m.created_at).toLocaleString()}</div>
                  </div>
                  <div className="text-sm text-[#0F172B]">{m.content || `[${m.type}]`}</div>
                  <div className="text-[11px] text-[#62748E] mt-1">by {m.sender?.display_name}</div>
                </li>
              ))}
            </ul>
          )}
