...
export default function AdminCheckInConfig() {
  ...
  // Fetch all users for deadline management
  ...
  const [activeTab, setActiveTab] = useState<'checklist' | 'deadlines'>('checklist');
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="font-[family-name:var(--font-display)] text-xl font-bold text-[#0F172B]">Check-In Configuration</h1>
        <p className="mt-1 text-sm text-[#62748E]">Configure checklist items per role and user deadline times</p>
      </div>
      ...
      <div className="mb-4 flex gap-1 rounded-lg bg-[#F1F5F9] p-1">
        <button
          ...
          className={`rounded-md px-4 py-1.5 text-sm font-medium transition ${
            activeTab === 'checklist' ? 'bg-white text-[#0F172B] shadow-sm' : 'text-[#62748E]'
          }`}
        >
          Role Checklists
        </button>
        <button
          ...
          className={`rounded-md px-4 py-1.5 text-sm font-medium transition ${
            activeTab === 'deadlines' ? 'bg-white text-[#0F172B] shadow-sm' : 'text-[#62748E]'
          }`}
        >
          User Deadlines
        </button>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#E2E8F0] text-left">
                ...
                <th className="px-5 py-3 text-xs font-medium text-[#62748E]">Actions</th>
              </tr>
            </thead>
            <tbody>
              {users.map((u: any) => {
