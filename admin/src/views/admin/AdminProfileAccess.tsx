...
export default function AdminProfileAccess() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Profile Access</h1>
        <p className="mt-1 text-sm text-slate-500">
          Email-gated public access to talent profiles. Synced with SquadHire (
          <code className="rounded bg-slate-100 px-1 py-0.5 text-xs">/talent-access</code>) — admins
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white p-3">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-56 rounded-md border border-slate-200 bg-white px-3 py-1.5 text-sm outline-none focus:border-slate-400"
          />
          <button
            onClick={() => {
              setEditing(null);
              ...
              {grants.map((g) => {
                ...
                return (
                  <tr key={g.id}>
                    ...
                    <td className="px-4 py-2.5 text-slate-500">{formatDate(g.expires_at)}</td>
                    <td className="px-4 py-2.5 text-slate-500">{formatDate(g.created_at)}</td>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-slate-400 disabled:bg-slate-50 disabled:text-slate-400"
            />
          </div>

          <div>
            <label className="mb-1 block text-xs font-medium text-slate-500">Categories</label>
            {catErr ? (
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-full px-3 py-1 text-xs font-medium transition ${
                        on ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-indigo-50'
                      }`}
                    >
                      {c.name}
                    </button>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-500">Expires on</label>
            <input
              type="date"
              value={expiresOn}
