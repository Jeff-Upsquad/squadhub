...
import AssignmentServiceDetail, { useAssignmentCatalog, useCreateAssignmentService } from './AdminAssignmentCatalog';
import type {
  AssignmentService,
  Subscription,
  SubscriptionPlan,
...
// Which product line the sidebar list is showing. Countries are shared by
// both catalogs, so that entry stays pinned above the switch.
type CatalogTab = 'subscription' | 'assignment';
...
export default function AdminSubscriptions() {
  ...
  const { data: assignmentServices, isLoading: assignmentsLoading } = useAssignmentCatalog();
  const createAssignmentService = useCreateAssignmentService();

  const subs: Subscription[] = listRes?.data || [];
  const services: AssignmentService[] = assignmentServices || [];
  const countries: Country[] = countriesRes?.data || [];
  const [tab, setTab] = useState<CatalogTab>('subscription');
  ...
  const activeService = useMemo(
    () => (selected?.type === 'assignment' ? services.find((s) => s.id === selected.id) || null : null),
    [services, selected],
  );

  // Switching tabs lands on that catalog's first entry, so the main pane never
  // shows the other product line's detail while the list shows this one's.
  function switchTab(next: CatalogTab) {
    setTab(next);
    if (next === 'subscription') {
      if (subs.length > 0) setSelected({ type: 'subscription', id: subs[0].id });
    } else if (services.length > 0) {
      setSelected({ type: 'assignment', id: services[0].id });
    }
  }

  const listLoading = tab === 'subscription' ? isLoading : assignmentsLoading;

  // The assignment line-up isn't the subscription line-up (e.g. Content), so
  // admins add services here rather than waiting on a migration.
  function addAssignmentService() {
    const name = prompt('Name the assignment service (e.g. Content)');
    if (!name || !name.trim()) return;
    createAssignmentService.mutate(name.trim());
  }
    ...
    <div className="-m-6 flex h-[calc(100vh)] overflow-hidden">
      ...
      <div className="flex w-56 shrink-0 flex-col border-r border-divider bg-surface">
        <div className="border-b border-divider px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-[family-name:var(--font-display)] text-sm font-bold text-foreground">Pricing catalog</h2>
          </div>
        </div>
        ...
        <nav className="flex flex-col gap-0.5 p-2">
          ...
          {/* Product line switch — subscriptions are priced per month, assignments per project */}
          <div className="mb-1 inline-flex overflow-hidden rounded-md border border-divider">
            {(['subscription', 'assignment'] as CatalogTab[]).map((t) => (
              <button
                key={t}
                onClick={() => switchTab(t)}
                className={`flex-1 px-2 py-1.5 text-[11px] font-semibold transition-colors ${
                  tab === t ? 'bg-ink text-white' : 'bg-surface text-foreground-muted hover:bg-canvas'
                } ${t === 'assignment' ? 'border-l border-divider' : ''}`}
              >
                {t === 'subscription' ? 'Subscriptions' : 'Assignments'}
              </button>
            ))}
          </div>
          ...
          ) : (tab === 'subscription' ? subs : services).map((s) => (
            <button
              key={s.id}
              onClick={() => setSelected({ type: tab, id: s.id } as SidebarItem)}
              className={`flex items-center justify-between rounded-md px-3 py-2 text-left text-[13px] transition-colors ${
                selected?.type === tab && selected.id === s.id
                  ? 'bg-[#EEF2FF] font-semibold text-accent'
                  : 'text-foreground-muted hover:bg-surface-alt'
              }`}
            >
          ...
          {tab === 'assignment' && !listLoading && (
            <button
              onClick={addAssignmentService}
              disabled={createAssignmentService.isPending}
              className="mt-1 rounded-md border border-dashed border-divider-strong px-3 py-2 text-left text-[13px] text-foreground-muted hover:bg-surface-alt disabled:opacity-50"
            >
              + Add service
            </button>
          )}
        ...
        <div className="mt-auto border-t border-divider p-3">
          <p className="text-[10px] text-foreground-dim">
            {tab === 'subscription'
              ? 'Catalog is fixed. Admin toggles plans / tiers / prices per country.'
              : 'Assignments carry no list price — set the margin we keep per level.'}
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto bg-surface-alt p-6">
        {selected?.type === 'countries' ? (
          ...
          <AssignmentServiceDetail key={activeService.id} service={activeService} countries={countries} />
        ) : activeSub ? (
