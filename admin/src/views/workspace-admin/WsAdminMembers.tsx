...
function roleBadgeColor(role: string) {
  switch (role) {
    ...
    case 'guest': return 'bg-surface-alt text-foreground-muted';
...
export default function WsAdminMembers() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <h2 className="mb-6 font-[family-name:var(--font-display)] text-2xl font-bold text-foreground">Members</h2>

      {/* Invite form */}
      <form onSubmit={handleInvite} className="mb-6 rounded-lg border border-divider bg-surface p-5">
        <h3 className="mb-3 font-[family-name:var(--font-mono)] text-[10px] font-medium uppercase tracking-[0.12em] text-foreground-muted">Invite Member</h3>
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 rounded-md border border-divider-strong bg-surface px-3 py-2 text-sm text-foreground placeholder-foreground-dim outline-none transition focus:border-accent focus:ring-1 focus:ring-accent"
          />
          <select
            value={inviteRole}
            onChange={(e) => setInviteRole(e.target.value)}
            className="rounded-md border border-divider-strong bg-surface px-3 py-2 text-sm text-foreground outline-none"
          >
      ...
      <div className="rounded-lg border border-divider bg-surface">
        ...
        {isLoading ? (
          <p className="p-5 text-sm text-foreground-dim">Loading members...</p>
        ) : (
          <div className="divide-y divide-divider">
            {members?.map((m) => (
              <div key={m.user_id} className="flex items-center gap-4 px-5 py-3">
                {/* Avatar */}
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-well text-sm font-medium text-foreground-muted">
                  {m.user.avatar_url ? (
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-foreground">
                    ...
                    {m.user_id === currentUser?.id && (
                      <span className="ml-1.5 text-xs text-foreground-dim">(you)</span>
                    )}
                  </p>
                  <p className="truncate text-xs text-foreground-dim">{m.user.email}</p>
                </div>
                ...
                {m.user_id !== currentUser?.id && (
                  <button
                    ...
                    className="rounded p-1 text-foreground-dim hover:bg-surface-alt hover:text-red-500"
                    title="Remove member"
                  >
