...
const nextConfig = {
  ...
  rewrites: async () => ({
    beforeFiles: [
      ...
      { source: '/channels/:path*', destination: 'http://server:4000/channels/:path*' },
      { source: '/messages/:path*', destination: 'http://server:4000/messages/:path*' },
      { source: '/users/:path*', destination: 'http://server:4000/users/:path*' },
      { source: '/dms/:path*', destination: 'http://server:4000/dms/:path*' },
