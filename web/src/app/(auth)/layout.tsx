...
export default function AuthLayout({
  ...
  // Desktop chrome (gradient, dots, theme toggle) hides itself on a phone
  // login via CSS :has(.msh-login), so this tree stays identical on server
  // and client and doesn't flash the wrong layout.
  return (
    <div className="auth-shell relative flex min-h-screen w-full items-center justify-center bg-[#F8FAFC] px-4 py-12 dark:bg-canvas">
      <div className="auth-desktop-chrome">
        <div
          aria-hidden
      ...
      <div className="auth-inner relative z-10 flex w-full max-w-md flex-col items-center gap-8">{children}</div>
    </div>
  );
}
