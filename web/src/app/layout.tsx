      ...
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            ...
            __html: `(function(){try{var f=new URLSearchParams(location.search).get('theme');if(f==='dark'||f==='light'){document.documentElement.classList.toggle('dark',f==='dark');return;}var t=localStorage.getItem('squadhub-theme');var p=t?JSON.parse(t):null;var pref=p&&p.state&&p.state.theme?p.state.theme:'auto';if(pref==='frost'){document.documentElement.dataset.theme='frost';return;}if(pref==='noir'){document.documentElement.classList.add('dark');document.documentElement.dataset.theme='noir';return;}var dark=pref==='dark'||(pref==='auto'&&window.matchMedia('(prefers-color-scheme: dark)').matches);if(dark)document.documentElement.classList.add('dark');}catch(e){}})();`,
          }}
        />
      </head>
