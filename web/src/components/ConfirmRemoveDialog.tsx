  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-sm rounded-xl border border-[var(--sh-hair)] bg-[var(--surface)] p-5 shadow-2xl">
        ...
        <p className="mt-1 text-sm text-[var(--sh-ink-3)]">{description}</p>

        <div className="mt-4 space-y-1.5">
          <label className="block text-xs font-medium text-[var(--sh-ink-3)]">
            Type <span className="font-mono font-semibold text-[var(--sh-ink)]">{confirmWord}</span> to confirm
          </label>
          <input
            type="text"
            value={input}
            ...
            autoFocus
            disabled={loading}
            className="w-full rounded-md border border-[var(--sh-hair)] bg-[var(--surface)] px-2 py-1.5 text-sm font-mono text-[var(--sh-ink)] focus:border-[var(--sh-ink)] focus:outline-none"
          />
        </div>
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button
            ...
            onClick={onClose}
            disabled={loading}
            className="rounded-md border border-[var(--sh-hair)] px-3 py-1.5 text-sm font-medium text-[var(--sh-ink)] hover:bg-[var(--sh-hair-3)] disabled:opacity-50"
          >
            Cancel
          </button>
          <button
