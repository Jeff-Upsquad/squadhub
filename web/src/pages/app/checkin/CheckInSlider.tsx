      ...
      <div className="fixed right-0 top-0 z-50 flex h-full w-96 flex-col bg-white shadow-xl animate-in slide-in-from-right duration-200">
            ...
            <div className="text-center">
              {result === 'on_time' && (
                <>
                  ...
                  <p className="mt-1 text-sm text-[#62748E]">Submitted on time. Great job!</p>
                </>
              )}
              {result === 'late' && (
                <>
              ...
              {result === 'error' && (
                <>
                  ...
                  <p className="mt-1 text-sm text-[#62748E]">A server error occurred. Please try again.</p>
                  <button
                    onClick={() => setResult(null)}
                    className="mt-4 rounded-lg border border-[#E2E8F0] px-4 py-2 text-sm text-[#62748E] hover:bg-[#F8FAFC]"
                  >
                    Try Again
                  </button>
                </>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {checklistItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <p className="text-sm text-[#90A1B9]">No checklist items configured for your role.</p>
              <p className="mt-1 text-xs text-[#90A1B9]">You can still submit an empty check-in.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {checklistItems
                ...
                .map((item) => (
                  <label
                    key={item.id}
                    className="flex cursor-pointer items-start gap-3 rounded-lg border border-[#E2E8F0] p-3 transition hover:bg-[#F8FAFC]"
                  >
                    <input
                      type="checkbox"
                      checked={checked.has(item.id)}
                      onChange={() => toggle(item.id)}
                      className="mt-0.5 h-4 w-4 rounded border-[#CBD5E1] text-[#0F172B] focus:ring-[#0F172B]"
                    />
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-[#0F172B]">{item.label}</span>
                        {item.isRequired && (
                          <span className="rounded bg-red-50 px-1.5 py-0.5 text-[9px] font-medium text-red-500">
                            Required
                          </span>
                        )}
                      </div>
                      {item.description && (
                        <p className="mt-0.5 text-xs text-[#90A1B9]">{item.description}</p>
                      )}
                    </div>
                  </label>
                ))}
        ...
        <div className="border-t border-[#E2E8F0] p-5">
          <button
            ...
            className="w-full rounded-lg bg-[#0F172B] py-2.5 text-sm font-medium text-white transition hover:bg-[#1D293D] disabled:opacity-50 disabled:cursor-not-allowed"
          >
