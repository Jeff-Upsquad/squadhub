...
function ChevronIcon({ open }: { open: boolean }) {
  return (
    <svg
      className={`h-3.5 w-3.5 shrink-0 text-[#555] transition-transform ${open ? 'rotate-90' : ''}`}
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
    >
...
function ListItem({ list, depth = 0 }: { list: List; depth?: number }) {
  ...
  return (
    ...
    >
      <svg className="h-3.5 w-3.5 shrink-0 text-[#555]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div>
      <div className="group flex items-center">
        <button
          ...
          className="flex flex-1 items-center gap-1.5 rounded-md px-3 py-1 text-left text-[13px] text-[#888] hover:bg-[#1a1a1a]"
        >
        ...
        <button
          ...
          className="mr-2 hidden text-[#555] hover:text-[#ededed] group-hover:block"
          title="Add list"
        >
          ...
          {adding && (
            <div className="px-3 py-1" style={{ paddingLeft: '44px' }}>
              <input
                ...
                className="w-full rounded border border-[#333] bg-[#1a1a1a] px-2 py-0.5 text-xs text-[#ededed] placeholder-[#555] outline-none focus:border-[#ededed]"
              />
            </div>
          )}
        </div>
    ...
    <div className="mb-1">
      <div className="group flex items-center">
        ...
        >
          <ChevronIcon open={open} />
          <span
            className="flex h-5 w-5 items-center justify-center rounded text-xs font-bold text-[#ededed]"
        ...
        <div className="mr-2 hidden gap-0.5 group-hover:flex">
          <button
            ...
            className="rounded p-0.5 text-[#555] hover:text-[#ededed]"
            title="Add folder"
          >
            <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="flex h-full w-full flex-col">
      <div className="flex items-center justify-between px-4 py-3">
        <h2 className="text-sm font-semibold text-[#ededed]">Spaces</h2>
        <button
          onClick={() => setShowCreate(true)}
          className="text-lg text-[#888] hover:text-[#ededed]"
          title="Create space"
        >
          +
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-2">
        {isLoading && (
          <p className="px-3 py-2 text-xs text-[#555]">Loading spaces...</p>
        )}
        {spaces?.length === 0 && !isLoading && (
          <div className="px-3 py-4 text-center">
            <p className="text-xs text-[#555]">No spaces yet</p>
            <button
              onClick={() => setShowCreate(true)}
              className="mt-2 text-xs font-medium text-[#ededed] hover:text-white"
            >
