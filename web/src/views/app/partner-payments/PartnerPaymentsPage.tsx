...
function CommissionChip({ status }: { status: 'paid' | 'processing' | 'pending' }) {
  ...
  if (status === 'processing') {
    return (
      <span className="inline-flex rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-semibold text-sky-700 dark:bg-sky-500/15 dark:text-sky-400">
        Processing
      </span>
    );
  }
...
function OverviewTab({
  ...
  const prevIdx = payouts.findIndex((p) => p.month === currentMonth);
  const prevMonth = prevIdx > 0 ? payouts[prevIdx - 1] : undefined;
  const pendingTotal = sumByCurrency(pending);
  // Previous month's payout plus anything still pending (the previous month
  // itself is excluded from the pending side so it is never double-counted).
  const prevPlusPending = sumByCurrency([
    ...(prevMonth ? [prevMonth] : []),
    ...pending.filter((p) => p.month !== prevMonth?.month),
  ]);
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        />
        <StatCard
          label="Previous month + pending"
          value={formatPayments(prevPlusPending)}
          sub={
            <span className="flex flex-wrap items-center gap-x-2 gap-y-1">
              {prevMonth && (
                <span className="inline-flex items-center gap-1.5">
                  {monthShort(prevMonth.month)} {formatPayments(prevMonth.payments)}
                  <CommissionChip status={prevMonth.commission_status} />
                </span>
              )}
              {pendingTotal.length > 0 && <span>Pending {formatPayments(pendingTotal)}</span>}
              {!prevMonth && pendingTotal.length === 0 && <span>No payouts yet</span>}
            </span>
          }
        />
...
function PayoutsTab({
  ...
  const pending = payouts.filter((p) => p.commission_status !== 'paid');
  const paid = payouts.filter((p) => p.commission_status === 'paid').reverse();

  return (
    <div className="space-y-4">
      <div className="overflow-hidden rounded-xl border border-divider bg-surface">
        <div className="flex items-center justify-between gap-3 border-b border-divider px-4 py-3">
          <h3 className="text-sm font-semibold">Not yet paid out</h3>
          <span className="text-sm font-bold tabular-nums">{formatPayments(sumByCurrency(pending))}</span>
        </div>
        <PayoutMonthTable
          rows={pending}
          open={open}
          ...
          emptyText="Nothing pending — everything is paid out."
        />
      </div>

      <div className="overflow-hidden rounded-xl border border-divider bg-surface">
        <div className="flex items-center justify-between gap-3 border-b border-divider px-4 py-3">
          <h3 className="text-sm font-semibold">Previous months earnings</h3>
          <span className="text-sm font-bold tabular-nums">{formatPayments(sumByCurrency(paid))}</span>
        </div>
        <PayoutMonthTable
          rows={paid}
          open={open}
          onToggle={toggle}
          currentMonth={currentMonth}
          emptyText="Nothing marked paid yet."
        />
      </div>
...
function PayoutMonthTable({
  ...
  if (rows.length === 0) {
    return <div className="px-4 py-8 text-center text-sm text-foreground-muted">{emptyText}</div>;
  }
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
