  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {/* Summary card */}
        {summary && (
          <div className="mb-6 rounded-xl border border-divider bg-surface-alt p-5">
            <div className="grid grid-cols-5 gap-4 text-center">
              <div>
                <p className="text-2xl font-bold text-foreground">{summary.total_working_days}</p>
                <p className="mt-0.5 text-[10px] uppercase tracking-wider text-foreground-dim">Working Days</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-emerald-600">{summary.on_time}</p>
                <p className="mt-0.5 text-[10px] uppercase tracking-wider text-foreground-dim">On Time</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-yellow-600">{summary.late}</p>
                <p className="mt-0.5 text-[10px] uppercase tracking-wider text-foreground-dim">Late</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-red-600">{summary.missed}</p>
                <p className="mt-0.5 text-[10px] uppercase tracking-wider text-foreground-dim">Missed</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-foreground">{summary.attendance_rate}%</p>
                <p className="mt-0.5 text-[10px] uppercase tracking-wider text-foreground-dim">Attendance</p>
              </div>
            </div>
          </div>
        )}
  ...
  return (
    <div className="grid grid-cols-7 gap-2">
      {days.map((day: any) => {
        ...
        return (
          <div key={day.date} className="flex flex-col items-center gap-2 rounded-lg border border-divider p-3">
            <span className="text-[10px] uppercase tracking-wider text-foreground-dim">{dayName}</span>
  ...
  return (
    <div>
      <div className="mb-2 grid grid-cols-7 gap-1 text-center">
        {DAY_NAMES.map((d) => (
          <span key={d} className="text-[10px] uppercase tracking-wider text-foreground-dim py-1">{d}</span>
        ))}
      </div>
  ...
  return (
    <div className="space-y-4">
      {Object.entries(months).map(([monthKey, monthDays]) => {
        ...
        return (
          <div key={monthKey} className="rounded-lg border border-divider p-4">
            <h4 className="mb-3 text-sm font-semibold text-foreground">{monthName}</h4>
            <div className="grid grid-cols-4 gap-3 text-center">
              <div>
                <p className="text-lg font-bold text-emerald-600">{onTime}</p>
                <p className="text-[10px] text-foreground-dim">On Time</p>
              </div>
              <div>
                <p className="text-lg font-bold text-yellow-600">{late}</p>
                <p className="text-[10px] text-foreground-dim">Late</p>
              </div>
              <div>
                <p className="text-lg font-bold text-red-600">{missed}</p>
                <p className="text-[10px] text-foreground-dim">Missed</p>
              </div>
              <div>
                <p className="text-lg font-bold text-gray-400">{holidays}</p>
                <p className="text-[10px] text-foreground-dim">Holidays</p>
              </div>
            </div>
          </div>
        );
