  ...
  return (
    <div className="p-5 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-[family-name:var(--font-display)] text-sm font-semibold text-[#0F172B]">
          Off Days
        </h3>
        {!showForm && (
          <button
            onClick={() => setShowForm(true)}
            className="rounded-lg bg-[#0F172B] px-3 py-1.5 text-xs font-medium text-white transition hover:bg-[#1D293D]"
          >
            + Request Off Day
          </button>
        )}
      ...
      {showForm && (
        ...
        >
          {/* Type selector */}
          <div>
            <label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-[#90A1B9]">
              Type
            </label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-[#E2E8F0] px-3 py-2 text-xs text-[#0F172B] focus:border-[#0F172B] focus:outline-none"
                />
              </div>
              <div>
                <label className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-[#90A1B9]">
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full rounded-lg border border-[#E2E8F0] px-3 py-2 text-xs text-[#0F172B] placeholder:text-[#90A1B9] focus:border-[#0F172B] focus:outline-none resize-none"
          ...
          <div className="flex gap-2">
            <button
              ...
              className="rounded-lg bg-[#0F172B] px-4 py-2 text-xs font-medium text-white transition hover:bg-[#1D293D] disabled:opacity-40"
            >
            ...
            <button
              type="button"
              onClick={resetForm}
              className="rounded-lg bg-[#F1F5F9] px-4 py-2 text-xs font-medium text-[#62748E] transition hover:bg-[#E2E8F0]"
            >
              Cancel
            </button>
          </div>
      ...
      {/* Requests list */}
      {isLoading ? (
        <p className="text-xs text-[#90A1B9]">Loading...</p>
      ) : requests.length === 0 ? (
        <div className="flex flex-col items-center py-10 text-center">
          <svg className="mb-3 h-10 w-10 text-[#E2E8F0]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
          </svg>
          <p className="text-xs text-[#90A1B9]">No off-day requests yet</p>
        </div>
      ) : (
          ...
          {requests.map((r: any) => (
            ...
            >
              <div className="space-y-1">
                ...
                {r.reason && (
                  <p className="text-[11px] text-[#90A1B9]">{r.reason}</p>
                )}
              </div>
              {r.status === 'pending' && (
                <button
                  ...
                  className="rounded-lg bg-red-50 px-3 py-1.5 text-[11px] font-medium text-red-600 transition hover:bg-red-100"
                >
                  Cancel
                </button>
              )}
