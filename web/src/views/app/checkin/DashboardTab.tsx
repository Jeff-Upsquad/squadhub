    ...
    <div className="flex-1 overflow-y-auto p-5">
      ...
      {summary && (
        <div className="mb-5 rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-4">
          <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-[#90A1B9]">Attendance</h4>
          <div className="grid grid-cols-5 gap-3 text-center">
            <div>
              <p className="text-xl font-bold text-[#0F172B]">{summary.total_working_days}</p>
              <p className="text-[9px] uppercase tracking-wider text-[#90A1B9]">Days</p>
            </div>
            <div>
              <p className="text-xl font-bold text-emerald-600">{summary.on_time}</p>
              <p className="text-[9px] uppercase tracking-wider text-[#90A1B9]">On Time</p>
            </div>
            <div>
              <p className="text-xl font-bold text-yellow-600">{summary.late}</p>
              <p className="text-[9px] uppercase tracking-wider text-[#90A1B9]">Late</p>
            </div>
            <div>
              <p className="text-xl font-bold text-red-600">{summary.missed}</p>
              <p className="text-[9px] uppercase tracking-wider text-[#90A1B9]">Missed</p>
            </div>
            <div>
              <p className="text-xl font-bold text-[#0F172B]">{summary.attendance_rate}%</p>
              <p className="text-[9px] uppercase tracking-wider text-[#90A1B9]">Rate</p>
            </div>
          </div>
        </div>
      )}
      ...
      {weekSummaries.length > 0 && (
        <div className="mb-5 rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-4">
          <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-[#90A1B9]">Weekly Hours</h4>
            ...
            {weekSummaries.map((day: any) => {
              ...
              return (
                <div key={day.date} className="flex items-center gap-3">
                  <span className="w-10 text-xs text-[#90A1B9]">{dayName} {dateNum}</span>
                  <div className="flex h-4 flex-1 overflow-hidden rounded-full bg-[#E2E8F0]">
                  ...
                  <span className="w-14 text-right text-xs font-medium text-[#0F172B]">{formatDuration(total)}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="mt-3 flex items-center justify-center gap-4">
            <div className="flex items-center gap-1.5"><div className="h-2.5 w-2.5 rounded-sm bg-blue-500" /><span className="text-[9px] text-[#90A1B9]">Work</span></div>
            <div className="flex items-center gap-1.5"><div className="h-2.5 w-2.5 rounded-sm bg-amber-400" /><span className="text-[9px] text-[#90A1B9]">Break</span></div>
            <div className="flex items-center gap-1.5"><div className="h-2.5 w-2.5 rounded-sm bg-gray-400" /><span className="text-[9px] text-[#90A1B9]">No Work</span></div>
          </div>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-7 gap-2">
          {days.map((day: any) => {
            ...
            return (
              <div key={day.date} className="flex flex-col items-center gap-1.5 rounded-lg border border-[#E2E8F0] p-2">
                <span className="text-[9px] uppercase tracking-wider text-[#90A1B9]">{dayName}</span>
                <span className="text-xs font-medium text-[#0F172B]">{dayNum}</span>
        ...
        <div>
          <div className="mb-2 grid grid-cols-7 gap-1 text-center">
            {DAY_NAMES.map((d) => (
              <span key={d} className="py-1 text-[9px] uppercase tracking-wider text-[#90A1B9]">{d}</span>
            ))}
          </div>
                ...
                day === null ? (
                  ...
                  <div
                    ...
                    className={`flex h-8 items-center justify-center rounded text-[10px] font-medium ${
                      day.status === 'on_time' ? 'bg-emerald-100 text-emerald-700'
                      : day.status === 'late' ? 'bg-yellow-100 text-yellow-700'
                      : day.status === 'no_checkin' ? 'bg-red-100 text-red-700'
                      : day.status === 'holiday' ? 'bg-gray-100 text-gray-400'
                      : 'text-gray-300'
                    }`}
      ...
      <div className="mt-4 flex items-center justify-center gap-3">
        {Object.entries(STATUS_LABELS).map(([key, label]) => (
          <div key={key} className="flex items-center gap-1.5">
            ...
            <span className="text-[9px] text-[#90A1B9]">{label}</span>
          </div>
        ))}
      </div>
    </div>
