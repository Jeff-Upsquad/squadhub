...
export default function CreateFolderListModal({
  ...
  const { data: profiles, isLoading: profilesLoading } = useAvailableProfiles(type);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      ...
      >
        <h2 className="mb-5 text-lg font-semibold text-[#0F172B] font-[family-name:var(--font-display)]">
          Create {type === 'folder' ? 'Folder' : 'List'}
        </h2>
        ...
        <input
          ...
          placeholder={type === 'folder' ? 'e.g. Design Assets' : 'e.g. Sprint Backlog'}
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {isPending ? 'Creating...' : `Create ${type === 'folder' ? 'Folder' : 'List'}`}
          </button>
        </div>
      </form>
    </div>
