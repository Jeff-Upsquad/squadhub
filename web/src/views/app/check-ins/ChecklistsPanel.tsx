      ...
      <div className="w-56 shrink-0">
        <div className="rounded-xl border border-divider bg-surface">
          <div className="border-b border-divider px-4 py-3">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-foreground-dim">Roles</h3>
          </div>
          <div className="p-2">
            {configs.map(({ role }) => (
              <button
          ...
          <div className="rounded-xl border border-divider bg-surface">
            <div className="flex items-center justify-between border-b border-divider px-5 py-3">
              ...
              <div className="flex gap-2">
                <button
                  onClick={addItem}
                  className="rounded-lg border border-divider px-3 py-1.5 text-xs font-medium text-foreground-muted hover:bg-surface-alt"
                >
                  + Add Item
                </button>
                {hasChanges && (
            ...
            {items.length === 0 ? (
              <div className="p-8 text-center text-sm text-foreground-dim">
                ...
                {items.sort((a, b) => a.order - b.order).map((item) => (
                  <div key={item.id} className="flex items-start gap-3 px-5 py-4">
                    <div className="flex flex-col gap-0.5 pt-1">
                      <button onClick={() => moveItem(item.id, 'up')} className="rounded p-0.5 text-foreground-dim hover:text-foreground">
                      ...
                      <button onClick={() => moveItem(item.id, 'down')} className="rounded p-0.5 text-foreground-dim hover:text-foreground">
                    ...
                    <div className="flex-1 space-y-2">
                      <input
                        ...
                        className="w-full rounded border border-divider px-3 py-1.5 text-sm focus:border-ink focus:outline-none"
                      />
                      <input
                        type="text"
                        ...
                        className="w-full rounded border border-divider px-3 py-1.5 text-xs text-foreground-muted focus:border-ink focus:outline-none"
                      />
                    ...
                    <button
                      ...
                      className="rounded p-1 text-foreground-dim hover:text-red-500"
                    >
