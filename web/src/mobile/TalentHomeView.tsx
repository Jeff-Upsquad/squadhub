...
export default function TalentHomeView() {
  ...
  return (
    <div className="space-y-4 bg-[#F5F5F6] p-3">
      {/* Hero — Welcome back (talent dashboard 1:1) */}
      <section className="relative overflow-hidden rounded-2xl border border-[#E7E7EA] bg-white px-5 py-5">
        ...
        <div className="relative">
          ...
          <h1 className="text-[22px] font-semibold tracking-[-0.025em] leading-[1.15] text-[#0a0a0a] sm:text-[24px]">
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-[#E7E7EA] bg-white px-5 py-8 text-center">
          <p className="text-sm font-semibold text-[#0a0a0a]">Couldn’t load</p>
          <p className="mt-1 text-sm text-[#737373]">Check your connection and try again.</p>
          <button type="button" onClick={() => refetch()} className="mt-3 rounded-full bg-[#0a0a0a] px-4 py-2 text-xs font-semibold text-white">Try again</button>
        </div>
      ) : visible.length === 0 ? (
