...
For server-only tables, explicitly revoke access from user-scoped roles before
granting access only to `service_role`. The revoke is required while older
Supabase defaults may still grant new public tables automatically:

```sql
REVOKE ALL PRIVILEGES
ON TABLE public.your_server_only_table
...
Do not grant `anon` or `authenticated` access unless the table is intentionally
queried through a user-scoped Data API client. Keep RLS enabled and define
policies for every table exposed to `anon` or `authenticated`.
