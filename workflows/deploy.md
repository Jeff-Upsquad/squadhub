...
6. **Data-fix migrations don't update the talent-facing copies on SquadHire.** The Client view (and talent cards) render `hours_label` and other content fields from SquadHire's stored mirror, frozen at the card's last publish. A migration that fixes plan/`plan_snapshot` data changes future publishes only — already-published cards keep showing stale values until re-delivered. To fix: run a one-off script inside the `squadhub-server` container on the VPS (local `server/.env` has no SquadHire creds) that reads each card's stored label via the signed POST `{SQUADHIRE_WEBHOOK_URL}/client-view/card` with `{external_id}` (response shape `{success, card, recipients}` — label is `card.hours_label`), then re-delivers stale cards with `buildSquadhirePayloadForCard` + `deliverCardToSquadhire` (idempotent, no re-broadcast). See the 2026-08-25 hours-rounding fix (migration 187) for the pattern.
